...
const addProduct = async ()=>{
  ...
  try {
     await axiosCustom.post(`cities`,formData, {
    });
        addmodal(false);
        fletchcard();
        name.value = "";
...
function sluginput(item){
  inputslug.value = item
}
...
function onChange(){
   ...
   data.value.slug = inputslug.value,
   data.value.images =  inputimages.value
   data.value.text = inputtext.value
}
...
const editChangeProduct = async ()=>{
 ...
 editmodal(false)
  try {
     await axiosCustom.put(`cities/${idtovar.value}`,formdata, {
    });
        fletchcard();
  } catch (error) {
  }
...
const deleteProduct = async ()=>{
  try {
    const response = await axiosCustom.delete(`cities/${idtovar.value}`)
    fletchcard();
    deletemodal(false)
  } 
  catch{
...
<template>
  <div class="citypages">
    <div class="citypages__container">
      <div class="citypages__container-add">
        <h3 class="citypages__container-add-text">Qo'shish</h3>
        <button class="citypages__container-add-btn" @click="modaladd = true">
          <svg
            class="btn-svg"
      ...
      <div class="modal__add__container">
        ...
        <form action="" @submit.prevent="addProduct">
          ...
          <input
            ...
            required
          />
          <textarea v-model="text" name="" id="" cols="30" rows="10" placeholder="text">
          </textarea>
    ...
    <div v-if="modaledit" class="modal__edit">
      <div class="modal__edit__container">
        ...
        <form action="" @submit.prevent="editChangeProduct">
          ...
          <textarea  v-model="data.text"  @input="textinput($event.target.value)" name="" id="" cols="30" rows="3"></textarea>
